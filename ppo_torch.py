...
class PPOMemory:
    def __init__(self, batch_size, buffer_len):
        self.states = deque(maxlen=buffer_len)
        self.probs = deque(maxlen=buffer_len)
        self.values = deque(maxlen=buffer_len)
        self.actions = deque(maxlen=buffer_len)
        self.rewards = deque(maxlen=buffer_len)
        self.dones = deque(maxlen=buffer_len)
...
class Agent:
    def __init__(self, n_actions, input_dims, gamma=0.99, alpha=0.0003, gae_lambda=0.98,
                 policy_clip=0.15, batch_size=64, buffer_len=100, n_epochs=10):
        self.gamma = gamma
        ...
        self.memory = PPOMemory(batch_size, buffer_len)
